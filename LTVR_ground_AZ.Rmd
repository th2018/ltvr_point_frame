...
# the highest number of each site-date-plot-pin will be the top (which the drone can see)
```{r}
pf_top <- pf %>%    
  group_by(site,Date,plot,pin ) %>%
   summarise(top=max(hit) )

pf_top_sp<-merge( pf,  pf_top, by.x=c( "site","Date","plot","pin","hit"), by.y=c( "site","Date","plot","pin","top" ))
pf_top_sp
...
pf_top_sp_count_los1_2019<-pf_top_sp_count[ pf_top_sp_count$site=="los1" & ,]
